---
name: session-end
description: 세션 종료 시 현재 세션의 컨텍스트를 요약하고 미완료 작업과 재사용 지식을 vault에 저장한다. 사용자가 세션 종료나 저장을 요청했을 때 실행하고, 긴 작업이 안전하게 넘길 수 있는 checkpoint에 도달했을 때는 한 번만 정리를 제안한다. 명시 요청이 없으면 동의 전에 파일을 쓰지 않는다.
---
...
사용자가 "/session-end", "세션 종료", "오늘 여기까지", "이 세션을 기록해줘", "지금까지를 기록해줘"처럼 현재 세션의 저장을 명시적으로 요청하면 바로 아래 workflow를 실행한다. 특정 오류나 내용을 README·문서 등에 기록해 달라는 요청은 session-end 실행 신호로 보지 않는다.

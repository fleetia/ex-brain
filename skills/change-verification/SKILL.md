...
상위 지침이 언어를 정하지 않은 경우 사용자에게 보이는 질문·진행 상황·결과는 사용자가 지정한 언어를 우선하고, 없으면 최근의 의미 있는 사용자 발화 언어를 따른다. 발화가 짧거나 혼합되어 모호하면 이미 형성된 대화 언어를 유지하며, code·command·path·identifier는 번역하지 않는다. 아래 검증 상태 label은 의미 구분이므로 선택한 응답 언어로 표현한다.

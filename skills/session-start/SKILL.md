---
name: session-start
description: 세션 시작 시 이전 세션의 컨텍스트를 복원한다. 사용자가 `/session-start`를 호출하거나, 사용 언어와 관계없이 이전 세션·진행 중 작업을 복원하거나 이어가려는 의도를 표현할 때 사용.
---
...
상위 지침이 언어를 정하지 않은 경우 사용자에게 보이는 질문·요약·상태 label은 사용자가 지정한 언어를 우선하고, 없으면 최근의 의미 있는 사용자 발화 언어를 따른다. 발화가 짧거나 혼합되어 모호하면 이미 형성된 대화 언어를 유지하며, code·command·path·identifier는 번역하지 않는다. 아래 출력 형식의 한국어 문구는 고정 문자열이 아니라 의미 예시이므로 선택한 응답 언어로 표현한다. 다만 vault frontmatter key와 `## 작업 내용`, `## 검증`, `## 작업 사본`, `## 미완료`, `## 과정 노트` 같은 canonical schema heading은 번역하지 않는다.
...
아래 heading과 상태 값은 선택한 응답 언어로 표현하되, 원본 vault schema는 그대로 유지한다.
